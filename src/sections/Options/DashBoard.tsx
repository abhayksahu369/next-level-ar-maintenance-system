...
type TimeRangeOption = '1h' | '6h' | '24h' | '7d' | '30d';
type ViewOption = 'realtime' | 'cumulative';

// Main component
const Dashboard: React.FC = () => {
  // State for storing sensor data with proper types
  const [sensorData, setSensorData] = useState<SensorDataPoint[]>([]);
  const [cumulativeData, setCumulativeData] = useState<SensorDataPoint[]>([]);
  const [selectedView, setSelectedView] = useState<ViewOption>('realtime');
  ...
  const vibrationChartRef = useRef<HTMLDivElement>(null);
  ...
  const filterDataByTimeRange = (data: SensorDataPoint[]): SensorDataPoint[] => {
    const now = new Date();
    const ranges: Record<TimeRangeOption, number> = {
      '1h': 60 * 60 * 1000,
      '6h': 6 * 60 * 60 * 1000,
      '24h': 24 * 60 * 60 * 1000,
      '7d': 7 * 24 * 60 * 60 * 1000,
  ...
  const createOrUpdateChart = (
    ...
    metric: keyof Pick<SensorDataPoint, 'temperature' | 'humidity' | 'vibration'>, 
    color: string
  ): void => {
  ...
  const getAverageValues = (): AverageValues => {
    const data = selectedView === 'realtime' ? sensorData : filterDataByTimeRange(cumulativeData);
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          ...
          <div className="bg-gray-800 rounded-lg shadow-lg p-4 border border-gray-700">
            ...
            <p className="text-3xl font-bold text-blue-400">{averages.vibration}m/s²</p>
          </div>
        </div>
